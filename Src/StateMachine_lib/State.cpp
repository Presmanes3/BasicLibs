...
void State::update() {
	...
	Action *action = this->update_actions.get_element(action_id);
...
void State::enter() {
	...
	for (uint32_t x = 0; x < this->enter_actions.get_size(); x++) {
		action = this->enter_actions.get_element(x);
...
void State::exit() {
	...
	for (uint32_t x = 0; x < this->exit_actions.get_size(); x++) {
		action = this->exit_actions.get_element(x);
...
ArrayPointerVector<Action>* State::get_action_vector(ActionType type) {
	switch (type) {
...
void State::add_action(ActionType type, Action* action_){
	ArrayPointerVector<Action>* vector = this->get_action_vector(type);

	vector->add_element(action_);
}
